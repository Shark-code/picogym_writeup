...
## Solution
In vuln.c we can see a format string vulnerability "printf(user_buf)" after we entered the API token. Then we can try to leak the flag on the stack. The flag will be in a reverse order.
